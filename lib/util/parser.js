...
class Json extends Base {
  ...
  stripDeletions (s) {
    // traverse and unset all keys marked for deletion before final write
    // this cant be done in prepare because we use the DELETE marker
    // in the diff
    traverse(s, (keys, value) => {
  ...
  toString (s) {
    // this can also be done in stringify replacer but sometimes we need
    // to do it in place without stringifying
    return JSON.stringify(this.stripDeletions(s), null, 2)
  }
...
class PackageJson extends JsonMerge {
  ...
  async write (s) {
    ...
    this.stripDeletions(pkg.update(s).content)
    pkg.save()
  }
}
