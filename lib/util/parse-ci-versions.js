...
// '^12.13 || ^14.15 || >=16'. This is not super generic but fits
...
const parseCITargets = (targets = []) => {
  ...
  const parsedRanges = uniq([...Object.keys(minVersions), ...Object.keys(minRanges)])
    ...
    .map((v, index, list) => {
      // change .0 to .x anywhere i the minor/patch portions of the version this
      // ensures that our engines will always include prereleases when tested with
      // `includePrerelease: true` in other places. This is because
      // >=18.0.0 will never include the prerelease 18.0.0-pre, but >=18.x.x will.
      const { major, minor, patch } = new semver.SemVer(v)
      const parts = [major]
      if (minor && patch) {
        parts.push(minor, patch)
      } else if (minor) {
        parts.push(minor)
      } else if (patch) {
        parts.push('x', patch)
      ...
      return (index === list.length - 1 ? '>=' : '^') + parts.join('.')
    })
