...
const { getData, mergeData, getRules } = require('./config.js')
...
const getWsPkgs = async (root, rootPkg) => {
  ...
  const { configWorkspaces, configOmitWorkspaces } = rootPkg.options
...
const runAll = async (root, { command, force }) => {
  ...
  const makeOptions = (pkg, rootData) => {
    const o = {
      ...
      options: {
        ...constants,
        isDogFood: rootPkg.options.isDogFood,
        needsUpdate: force || rootPkg.options.isDogFood || pkg.options.needsUpdate,
      },
    }
  ...
  for (const pkg of pkgs) {
    ...
    for (const { when, run, name, rule, baseDirs, data: ruleData } of rules.flat()) {
      ...
      ruleOptions.pkg.data = mergeData([{ values: ruleOptions.pkg.data }, ruleData()]).data
      ruleOptions.rootPkg.data = mergeData([{ values: ruleOptions.rootPkg.data }, ruleData()]).data
